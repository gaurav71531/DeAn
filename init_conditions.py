...
# the data, shuffled and split between tran and test sets
	num_repetations = 4 # applying repetations to the pool data
	noise_var = 0.1
	...
	train_data_pool = train_data_pool.float()
	train_data_pool /= 255
	train_data_pool += torch.from_numpy(np.random.normal(0, noise_var, np.shape(train_data_pool))).float()
...
def getEMNISTData(batch_size = 128, trainInitSize = 2*47):
...
# the data, shuffled and split between tran and test sets
	...
	test_dataset = datasets.EMNIST('../data', train=False, 
					transform=transforms.ToTensor(),
					split= "balanced")
	test_loader = torch.utils.data.DataLoader(test_dataset,
			batch_size=len(test_dataset), shuffle=False, **kwargs)
	...
	train_data_val = train_data_all[-n_test_data:, :,:] # balanced validation dataset
	train_target_val = train_target_all[-n_test_data:]
	train_data_pool = train_data_all[20000:80000, :,:]
	train_target_pool = train_target_all[20000:80000]
	...
	train_data = []
	train_target = []
	numInit = np.floor(trainInitSize/47)*47
	numEachClass = int(numInit/47)
	...
	return torch.from_numpy(train_data/255).float(), torch.from_numpy(train_target), \
		 train_data_val/255,train_target_val, test_data/255, test_target, \
		 train_data_pool/255, train_target_pool, 47
...
def getCIFAR100Data(trainInitSize = 500):
	...
	numInit = np.floor(trainInitSize/100)*100
	numEachClass = int(numInit/100)
